    ...
    if response['response_continue'] =='STOP' :
        if response["action"]["action_number"] == 1:
          ...
          return education_agent.generate_education_agent_response(response['user_intent_turkish'])
            ...
            return fuel_price_agent.generate_fuel_price(response["action"]['city_name'])
        elif response["action"]["action_number"] == 5:
            return transporation_agent.generate_transport_price_response(response["action"]['city_name'])
